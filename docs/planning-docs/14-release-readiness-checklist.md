...
- [~] Validate every IPC payload with shared schemas or explicit type guards instead of unchecked casts. A shared validation module now covers settings, search, export, backup, duplicates, collections, tags, custom fields, files, devices, and dialogs; media remains open.
